...
export const getTripsByTravelStyle = async () => {
    ...
    const travelStyleCounts = trips.documents.reduce(
        (acc: { [key: string]: number }, trip: Document) => {
            const details = trip.tripDetails ?? trip.tripDetail;
            if (!details) return acc;
            const tripDetail = parseTripData(details);
...
export const getUserTripCounts = async () => {
    const trips = await database.listDocuments(
        appwriteconfig.databaseId,
        appwriteconfig.tripCollectionId
    );
    ...
    const counts = trips.documents.reduce(
        (acc: { [key: string]: number }, trip: Document) => {
            const userId = trip.userId || trip.accountId;
            if (userId) {
